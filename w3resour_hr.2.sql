...
select * from actor ;  select * from actor_award ; 		select * from customer ;	select * from address ;		select * from city ;	select * from country;
select* from film; 	select * from address ; 	select * from advisor ;		select * from rental ; 		select * from payment ;		select * from inventory ;
...
select * from countries;  select * from departments;  select * from employees;  select * from job_history;  select * from jobs;  select * from locations;  
select * from regions;
...
where e.salary=(select min_salary from jobs j
...
where (1) = (select count(distinct e2.salary) from employees e2
...
where (5)= (select count(distinct e2.salary) from employees e2
where e2.salary>=e1.salary) 
order by e1.salary
...
where e.department_id=d.department_id) dep
from employees e
order by dep;

select employee_id, first_name, d.department_id, d.department_name  from employees e
...
where not exists(select 'x' from employees b
...
all the Shipping Clerk (JOB_ID = 'SH_CLERK'). 
...
-- SQL JOINS QUESTIONS on w3resources------------
...
4. From the following tables, write a SQL query to find all those employees who work in department ID 80 or 40. Return first name, last name, department number 
and department name.
SELECT E.first_name , E.last_name , 
       E.department_id ,  D.department_name 
         FROM employees E 
...
5. From the following tables, write a SQL query to find those employees whose first name contains the letter ‘z’. Return first name, last name, department, city, 
and state province.
SELECT E.first_name,E.last_name,
   D.department_name, L.city, L.state_province
     FROM employees E 
...
11. From the following table, write a SQL query to find the employees and their managers. Those managers do not work under 
...
12. From the following tables, write a SQL query to find the employees who work in the same department as the employee 
...
14. From the following tables, write a SQL query to calculate the difference between the maximum salary of
 the job and the employee's salary. Return job title, employee name, and salary difference.
...
15. From the following table, write a SQL query to calculate the average salary, the number of employees receiving commissions in
 that department. Return department name, average salary and number of employees. 
...
16. From the following tables, write a SQL query to calculate the difference between the maximum salary and the salary of all the employees who work in 
the department of ID 80. Return job title, employee name and salary difference.
...
24. From the following tables, write a SQL query to find full name (first and last name), and salary
...
25. From the following tables, write a SQL query to find full name (first and last name), job title, start and end date of last jobs 
of employees who did not receive commissions. 
